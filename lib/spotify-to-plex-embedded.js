...
export const buildSpotifyToPlexEmbeddedSupervisordConf = ({
    ...
    const sharedEnv = [
        ...
        ...envPairs.filter((line) => !line.startsWith('TZ=') && !line.startsWith('SYNC_ON_STARTUP=')),
    ].join(',');
...
autostart=true
autorestart=true
environment=PORT="${SPOTIFY_TO_PLEX_EMBEDDED_PORT}",HOSTNAME="127.0.0.1",SPOTIFY_SCRAPER_URL="http://127.0.0.1:3020",${sharedEnv}
priority=200
startsecs=15
startretries=3
...
const stopSupervisor = (log = () => {}) => {
    if (!supervisorChild || supervisorChild.killed) {
    ...
    try {
        ...
        log('[spotify-sync] Stopped bundled spotify-to-plex supervisor');
    } catch (error) {
        log(`[spotify-sync] Failed to stop bundled supervisor: ${error.message}`);
        stoppingIntentionally = false;
    }
    supervisorChild = null;
};
...
const startSupervisor = async ({ configDir, config, log = () => {} } = {}) => {
    ...
    supervisorChild.on('exit', (code, signal) => {
    ...
    while (Date.now() < deadline) {
        if (!supervisorChild || supervisorChild.exitCode != null) {
            throw new Error('Bundled spotify-to-plex supervisor failed to start. Check portal logs.');
        }
        try {
    ...
    throw new Error('Bundled spotify-to-plex did not become ready in time.');
};

export const getSpotifyToPlexEmbeddedStatus = () => ({
    running: !!(supervisorChild && !supervisorChild.killed && supervisorChild.exitCode == null),
 ...
 */
export const syncSpotifyToPlexEmbeddedWorker = async (config, { configDir, log = () => {} } = {}) => {
    ...
    if (supervisorChild && !supervisorChild.killed && supervisorChild.exitCode == null) {
        stopSupervisor(log);
        await new Promise((r) => setTimeout(r, 500));
    }

    if (startingPromise) {
        try { await startingPromise; } catch { /* ignore prior failure */ }
    }

    startingPromise = startSupervisor({ configDir, config, log });
    try {
