...
export const createOverlaysRouter = ({
    ...
    router.get('/kometa', async (_req, res) => {
        try {
            ...
            res.status(500).json({ error: error.message || 'Failed to list Kometa overlays' });
        }
    });
    ...
    router.post(
        ...
        async (req, res) => {
            try {
                ...
                const kind = String(req.query?.kind || req.headers['x-overlay-kind'] || 'season').toLowerCase() === 'episode'
                    ? 'episode'
                    : 'season';
                const original = String(req.query?.name || req.headers['x-overlay-name'] || 'banner');
                const id = sanitizePresetUploadName(kind, original);
    ...
    router.post('/revert-kometa', async (req, res) => {
        try {
            ...
            pushActivity(
                ratingKey
                    ? `Reverting Kometa overlay for item ${ratingKey}…`
                    : 'Reverting ALL Kometa overlays…',
                'warn',
            );
            const result = await withPlexCli('revert-kometa', ratingKey ? { ratingKey } : {});
            pushActivity(
                `Kometa revert complete — ${result.result?.reverted ?? 0}/${result.result?.requested ?? 0} restored`,
                'warn',
            );
            res.json({ ok: true, ...(result.result || {}), logs: result.logs });
        } catch (error) {
            res.status(error.status || 500).json({ error: error.message || 'Kometa revert failed', logs: error.logs || [] });
        }
    });
    ...
    router.post('/reset-all', async (_req, res) => {
        try {
            ...
            pushActivity('Resetting all logged show + episode overlays…', 'warn');
