...
export const createOverlaysRouter = ({
    ...
    router.post('/run', async (req, res) => {
        try {
            ...
            const out = await runCommand(command, { previewMode: preview });
            res.json(out);
        } catch (error) {
    ...
    router.post('/preview', async (_req, res) => {
        try {
            const out = await runCommand('preview', { previewMode: true });
            res.json(out);
        } catch (error) {
            res.status(error.status || 500).json({
    ...
    router.post('/stop', async (_req, res) => {
        ...
        pushActivity(killed ? 'Stopped active overlays worker' : 'No active worker to stop', killed ? 'warn' : 'info');
