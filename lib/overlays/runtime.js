...
export const getOverlaysRuntimeStatus = async () => {
    ...
    return {
        ...
        episodeLogCount: Object.keys(episodeLog).length,
...
export const listOverlaysEpisodes = async () => {
    ...
    const episodes = Object.entries(log).map(([ratingKey, entry]) => ({
        ratingKey,
        title: entry?.title || ratingKey,
    ...
    episodes.sort((a, b) => String(b.timestamp || '').localeCompare(String(a.timestamp || '')));
    return { ok: true, episodes, total: episodes.length };
};
