...
export const DEFAULT_OVERLAY_PLACEMENT = {
    ...
    season: {
        ...
        anchorX: 'center',
        anchorY: 'bottom',
        // Same New Episode PNG as episode thumbs — keep full rounded corners.
        bottomClip: 0,
    },
    episode: {
        x: 0.5,
...
export const isNewSeasonPickerExcludedPresetId = (id) => {
    const name = String(id || '').toLowerCase();
    return name === 'recently-added' || isModeOnlyPresetId(name);
};
...
export const normalizeOverlaysPlacement = (input = {}) => {
    const raw = input && typeof input === 'object' ? input : {};
    const keys = ['show', 'season', 'episode', 'media', 'status', 'ratings', 'network', 'recently'];
    const out = {};
    ...
    // Migrate stock New Episode badge targets off the old 10% bottom clip so
    // season posters and episode thumbs keep the PNG's rounded corners.
    for (const key of ['season', 'episode']) {
        const clip = Number(out[key]?.bottomClip);
        if (Number.isFinite(clip) && Math.abs(clip - 0.1) < 0.001) {
            out[key] = { ...out[key], bottomClip: 0 };
        }
    }
    return out;
};
