...
const normalizeCustomCollectionRule = (raw = {}) => {
    ...
    const id = String(row.id || '').trim()
        || `rule-${String(row.collectionRatingKey || row.collection_rating_key || Date.now()).trim() || 'x'}`;
    const collectionRatingKey = String(row.collectionRatingKey || row.collection_rating_key || '').trim();
    ...
    return {
        ...
        collectionRatingKey,
        collectionTitle: String(row.collectionTitle || row.collection_title || '').trim(),
        library,
        librarySectionId,
        image,
    };
...
export const normalizeCustomCollectionOverlays = (input) => {
    ...
    for (const row of input) {
        ...
        if (!rule.collectionRatingKey || !rule.image || !rule.library) continue;
