...
const normalizeCustomCollectionRule = (raw = {}) => {
    ...
    const library = String(row.library || row.libraryTitle || row.library_title || '').trim();
    const librarySectionId = String(
...
export const normalizeCustomCollectionOverlays = (input) => {
    ...
    for (const row of input) {
        ...
        if (!rule.collectionRatingKeys.length || !rule.image || !rule.library) continue;
