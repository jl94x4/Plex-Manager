...
export const createPlaylistArtworkApplier = ({
    ...
    return ({ ratingKey, imageUrls } = {}) => applyPlexPlaylistArtwork({
        ...
        uploadPoster: async ({ ratingKey: id, buffer, contentType }) => {
            const url = `${base}/library/metadata/${encodeURIComponent(id)}/posters`;
            const response = await fetchImpl(url, {
                method: 'POST',
                headers: {
                    ...plexHeaders(token, {
            ...
            if (!response?.ok) {
                throw new Error(`Plex poster HTTP ${response?.status || 0}`);
            }
        },
    });
};
