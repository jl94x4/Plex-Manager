...
test('notification badge inverts a dark circular plate to the light mark', () => {
    ...
    assert.ok(pixelAt(out, 32, 10).a < 40, 'dark plate should be punched out');
    const mark = pixelAt(out, 32, 32);
    assert.equal(mark.r, 255);
    assert.equal(mark.g, 255);
    assert.equal(mark.b, 255);
    assert.ok(mark.a > 200, 'light letters should be the status-bar mask');
});
