...
export const registerAchievementsRoutes = (app, deps) => {
    ...
    const gatherHistoryAndStats = async (req, config, accountId, identity = {}, opts = {}) => {
        ...
        const maybeEnrich = async (historyItems, lookups = genreLookups) => {
            ...
            if (maxLookups > 0 && typeof fetchPlexMetadataGenres === 'function') {
            ...
            if (isTautulliWatchHistorySource(config) && typeof fetchTautulliUserHistoryItems === 'function') {
                try {
                    ...
                    if (historyItems?.length) {
                        ...
                        historyItems = await maybeEnrich(historyItems, Math.max(genreLookups, 400));
                        return {
                            historyItems,
        ...
        try {
            ...
            const attachPeriod = async (payload, historyItems, timeZone, evaluation = null) => {
                ...
                if (Number.isFinite(daysNum) && daysNum > 0 && String(daysParam) !== 'all') {
                    ...
                    priorPeriodXp = computeXpBreakdown(
                        priorStats,
                        normalizeXpWeights(config.achievementsXpWeights),
                    ).xp;
                }
                ...
                const periodBadgesEarned = earnedList.filter((b) => {
                    const at = Date.parse(b?.earnedAt || '');
            ...
            if (!forceFresh && hasSnapshot && !wantPeriod) {
            ...
            if (!forceFresh && wantPeriod && hasSnapshot) {
                const cachedEval = evaluateFromSnapshot(accountId, prev, config);
    ...
    app.get('/api/achievements/leaderboard', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            const entries = (Array.isArray(value?.entries) ? value.entries : []).map((entry, index) => {
                ...
                if (!obfuscate) return { ...entry, isMe };
                return {
                    ...entry,
                    isMe,
            ...
            if (accountId && viewerThumb) {
                for (const entry of entries) {
                    if (String(entry.accountId) === String(accountId)) entry.thumb = viewerThumb;
                }
            }
    ...
    app.get('/api/achievements/badge/:badgeId', requireAuth, requireMember, async (req, res) => {
        try {
            ...
            res.json({
                ...
                seasonActive: true,
                unlockCount: holders.length,
                totalUsers: users.filter((u) => u && !u.leaderboardOptOut).length,
                you: {
                ...
                earliestHolders: holders.slice(0, 8),
