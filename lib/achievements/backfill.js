...
export const ensurePortalAchievementsBackfill = async (deps = {}, opts = {}) => {
    ...
    const run = (async () => {
        ...
        if (adminPlexId && nextUsers[adminPlexId] && targets.some((t) => t.accountId === '1')) {
            delete nextUsers[adminPlexId];
        }
        ...
        if (historySource === 'tautulli' && typeof fetchTautulliUserHistoryItems === 'function') {
            ...
            const workers = Array.from({ length: concurrency }, async () => {
                while (cursor < toScore.length) {
                    ...
                    if (items.length && typeof fetchPlexMetadataGenres === 'function') {
                        if (!plexUriForFallback && typeof getPlexConnectionUri === 'function') {
