...
const scoreTarget = ({
    ...
    const stats = historyItems?.length
        ? buildStatsFromHistoryItems(historyItems, {
            timeZone: config.portalTimezone || process.env.PORTAL_TIMEZONE || process.env.TZ || 'UTC',
        })
...
export const ensurePortalAchievementsBackfill = async (deps = {}, opts = {}) => {
    ...
    const run = (async () => {
        ...
        for (const user of users || []) {
            // Best-effort map for emails when scoring via Tautulli.
            const id = String(user?.plexAccountId || '').trim();
            if (id) portalByAccountId[id] = user;
        }
                ...
                while (cursor < toScore.length) {
                    ...
                    try {
                        items = await fetchTautulliUserHistoryItems(config, {
                            ...
                            email: portal.email,
                            plexAccountName: target.username || portal.username,
                            maxItems: 100000,
                        });
                    } catch {
                        items = [];
        ...
        return { ok: true, processed: toScore.length, missingBefore: missing.length, historySource };
    })();
