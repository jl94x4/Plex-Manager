...
/** Fill show year + TMDB from series metadata — episode recently-added rows often omit both. */
...
export const browsePlexLibraryMedia = async (config, deps, options = {}) => {
    ...
    const maxTake = options.allowLarge === true ? 5000 : 120;
...
export const browseJellyfinLibraryMedia = async (config, deps, options = {}) => {
    ...
    const params = new URLSearchParams({
        ...
        Fields: 'ProductionYear,DateCreated,PrimaryImageAspectRatio,ImageTags,ProviderIds',
    });
...
const mapJellyfinBrowseItem = (config, item = {}, mediaType = 'movie', withBasePath) => {
    ...
    return {
        ...
        mediaType,
        tmdbId: item?.ProviderIds?.Tmdb != null ? String(item.ProviderIds.Tmdb) : null,
        thumb: posterId,
        thumbUrl: posterId
...
export const fetchPlexItemTmdbId = async (config, deps, ratingKey) => {
    ...
    const meta = data?.MediaContainer?.Metadata?.[0];
    return extractTmdbIdFromPlexMeta(meta || {});
};

const pageAllLibraryBrowse = async (browsePage, options = {}) => {
    const pageSize = Math.min(Math.max(Number(options.pageSize) || 200, 50), 500);
    const maxItems = Math.min(Math.max(Number(options.maxItems) || 8000, pageSize), 12000);
    const items = [];
    let start = 0;
