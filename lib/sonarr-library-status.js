...
export const fetchSonarrLibraryStatusForShow = async (config, tmdbId, opts = {}) => {
    ...
    // Stats can only confirm completeness when episode-level aired counts exist.
    if (!showComplete && totalAiredEpisodes === 0) {
        showComplete = false;
    } else if (!showComplete && isShowCompleteFromStatistics(series) && airedMain.length === 0 && mainSeasons.length === 0) {
        showComplete = false;
    }
