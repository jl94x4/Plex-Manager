...
/**
 * Punch a dark circular plate so the light mark (SZ letters, bat, etc.) is the
 * status-bar mask. Inverse of treating the whole opaque badge as the icon.
 */
const plateKnockoutMask = (rgba, size, opaqueIdx) => {
    if (opaqueIdx.length < 16) return null;
    const lums = opaqueIdx.map((i) => luminance(rgba[i * 4], rgba[i * 4 + 1], rgba[i * 4 + 2]));
    const sorted = [...lums].sort((a, b) => a - b);
    const median = sorted[Math.floor(sorted.length / 2)];
    const keepLight = median < 120;
    const keepDark = median > 160;
    if (!keepLight && !keepDark) return null;

    const mask = Buffer.alloc(size * size);
    let kept = 0;
    for (let k = 0; k < opaqueIdx.length; k += 1) {
        const i = opaqueIdx[k];
        const alpha = keepLight
            ? clampByte(((lums[k] - 90) / 80) * 255)
            : clampByte(((160 - lums[k]) / 80) * 255);
        mask[i] = alpha;
        if (alpha > 128) kept += 1;
    }
    const frac = kept / opaqueIdx.length;
    if (frac < 0.06 || frac > 0.62) return null;
    return mask;
};
...
export const makeNotificationBadgePng = (inputBuffer, size = 96) => {
    ...
    const plateMask = plateKnockoutMask(rgba, s, opaqueIdx);
    if (plateMask) {
        for (let i = 0; i < s * s; i += 1) {
            writeWhite(i * 4, plateMask[i]);
        }
        return PNG.sync.write(out);
    }
