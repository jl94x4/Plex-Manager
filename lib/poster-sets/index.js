...
export const createPosterSetsRouter = ({
    ...
    router.post('/preview', async (req, res) => {
        try {
            ...
            if (run.ok) {
                if (isTpdbUrl(url) && config.tpdbLocalCacheEnabled !== false) {
                    try {
                        ...
                        if (config.tpdbAggressivePrefetch !== false) {
    ...
    router.post('/search', async (req, res) => {
        try {
            ...
            if (titleSources?.length) {
                ...
                const cacheEnabled = posterSetsConfig.tpdbLocalCacheEnabled !== false;
    ...
    router.post('/tpdb-cache/warm-library', async (req, res) => {
        try {
            const config = await loadPosterSetsConfig();
            if (config.tpdbLocalCacheEnabled === false) {
            ...
            for (const item of items.slice(0, 200)) {
                const itemTmdb = String(item?.tmdbId || item?.id || '').trim();
                if (!itemTmdb) continue;
                const itemMedia = resolvePosterSetsMediaType(item?.mediaType, 'movie');
                const titleHint = String(item?.title || '').trim();
                const yearHint = item?.year ?? null;
                queuedTitles += 1;
                void (async () => {
                    try {
                        ...
                        const hydrate = await enqueueTpdbLibraryTitleHydrate(result.sets || [], {
                            libraryScoped: true,
                            tmdbId: itemTmdb,
                            force: true,
                        });
                        queuedSets += hydrate.queued || 0;
                    } catch {
                        /* warm is best-effort */
                    }
                })();
            ...
            res.json({
                ...
                started: true,
                titles: queuedTitles,
                message: `Warming ThePosterDB cache for ${queuedTitles} library title(s). Only library titles are scraped.`,
            });
        } catch (error) {
        ...
        try {
            ...
            if (job.type === 'apply' && (applyDest === 'jellyfin' || applyDest === 'emby')) {
                ...
                if (
                    ...
                    && config.tpdbLocalCacheEnabled !== false
                ) {
