        ...
        try {
            ...
            res.json({
                ...
                message: warmForcePublicSearch
                    ? `Queued ${queuedCount} library title(s) for public-search cache build (no TPDB login).${skipNote}`
                    : hasTpdbCredentials(config)
                        ? `Queued ${queuedCount} library title(s) for cache build${config.tpdbWarmParallelWorkers === true ? ' (5 parallel workers)' : ''} — set lists only; images hydrate on open.${skipNote}`
