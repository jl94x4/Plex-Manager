...
export const createPosterSetsRouter = ({
    ...
    /** One Python process / one TPDB login for a chunk of library titles. */
    const warmBatchRunner = async (items = []) => {
        const config = await loadPosterSetsConfig();
        const enriched = [];
        ...
        try {
            ...
            if (!hasTpdbCredentials(config)) {
                ...
                try {
                    ...
                    if (!loginProbe.ok) {
                        ...
                        logTpdbCacheActivity(
                            `Warm: TPDB login probe failed — ${probeError}`,
                            { level: 'error', current: false },
                        );
                        return res.status(400).json({
                            error: `TPDB login failed before Warm started — ${probeError}. Fix credentials (Settings → Test) then try again.`,
                        });
                    }
                    ...
                    return res.status(400).json({
                        error: `TPDB login probe error — ${probeError?.message || probeError}`,
                    });
                }
            }
            ...
            res.json({
                ...
                message: hasTpdbCredentials(config)
                    ? `Queued ${queuedCount} library title(s) for batch Warm (one login, serial title resolves, parallel image downloads).${skipNote}`
