    ...
    router.post('/test', async (req, res) => {
        try {
            ...
            if (config.tpdb_username && config.tpdb_password && config.tpdb_password !== '********') {
                ...
                tpdb = tpdbRun.ok ? (tpdbRun.result || {}) : { ok: false, error: tpdbRun.error || 'TPDB login failed' };
            }
        ...
        try {
            ...
            if (titleSources?.length) {
                ...
                const linkedTmdb = titleSources.find((source) => (
                    ...
                    || tmdbId
                    || null;
                const externalIds = linkedTmdb
                    ? await fetchTmdbExternalIds(linkedTmdb, mediaType, loadPortalConfig)
                    : { imdbId: null, tvdbId: null };
            ...
            if (provider === 'both') {
                ...
                let mediuxTitles = [];
                try {
                    const mediuxResult = await searchMediuxTitles();
                ...
                try {
                    const posterdbResult = await runCliSearch('posterdb', {
                        tmdbId: linkedTmdb,
                        imdbId: externalIds.imdbId || imdbId || null,
                        tvdbId: externalIds.tvdbId || null,
                        titleHint,
                    ...
                    titles.push(...(posterdbResult.titles || []));
