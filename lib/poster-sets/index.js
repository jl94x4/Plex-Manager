...
export const createPosterSetsRouter = ({
    ...
    const collectLibraryWarmItems = async ({
        ...
        const seen = new Set();
        const out = [];
        const wantMedia = media === 'movie' || media === 'show' ? media : '';

        const pushRow = (row) => {
            if (out.length >= limit) return;
            const tmdbId = String(row?.tmdbId || row?.id || '').trim();
            if (!/^\d+$/.test(tmdbId)) return;
            const mediaType = String(row?.mediaType || row?.type || 'movie').toLowerCase() === 'show'
                ? 'show'
                : 'movie';
            if (wantMedia && mediaType !== wantMedia) return;
            const key = coverageKeyForLibraryItem({ tmdbId, mediaType });
            if (!key || seen.has(key) || exhausted.has(key)) return;
            if (skipCached && cachedKeys.has(key)) return;
            seen.add(key);
            out.push({
                tmdbId,
    ...
    router.post('/tpdb-cache/warm-library', async (req, res) => {
        try {
            ...
            const items = Array.isArray(req.body?.items) ? req.body.items : [];
            const media = req.body?.media === 'movie' || req.body?.media === 'show' ? req.body.media : 'all';
            const source = req.body?.source === 'recent' ? 'recent' : 'full';
            const force = req.body?.force === true || req.body?.skipCached === false;
            ...
            const queued = await enqueueTpdbLibraryWarm(normalized, async (item) => {
                ...
                warmSuccesses += 1;
                return result;
            }, { force, followedPrefetchOnly, autoContinue: true, resetExhausted: fromLibrary || force });
