...
export const createPosterSetsRouter = ({
        ...
        try {
            ...
            const runCliSearch = async (providerId, extra = {}, { onBatch } = {}) => {
                const payload = {
                    ...
                    mediaType: resolvePosterSetsMediaType(extra.mediaType ?? mediaType, 'movie'),
                    tmdbId: extra.tmdbId ?? tmdbId,
                    imdbId: extra.imdbId ?? imdbId,
                    tvdbId: extra.tvdbId ?? req.body?.tvdbId ?? req.body?.tvdb_id ?? null,
            ...
            if (titleSources?.length) {
                ...
                const settled = await Promise.allSettled(titleSources.map(async (source) => {
                    ...
                    return runCliSearch('posterdb', {
                        ...
                        titleUrl: linkedTmdb ? '' : staleTitleUrl,
                        tmdbId: linkedTmdb,
    ...
    const runQueuedJob = async (job) => {
        ...
        const onProgress = async (message) => {
            ...
            const logs = [
                ...
                { at: new Date().toISOString(), message: String(message || '') },
            ].slice(-500);
        ...
        try {
            ...
            if (job.type === 'apply' && (applyDest === 'jellyfin' || applyDest === 'emby')) {
                ...
                const result = await applyPosterSetToJellyfinLike({
                    ...
                    onProgress: (message) => { void onProgress(message); },
                });
    ...
    router.post('/queue/clear-queued', async (_req, res) => {
        try {
            const { state, cancelled } = await clearPosterSetsQueuedJobs();
            for (const job of state.jobs) {
                if (job.state === 'cancelled' && job.error === 'Cancelled while queued (clear queued)') {
                    mirrorJob(job);
    ...
    router.post('/watches/run', async (_req, res) => {
        ...
        if (watchPassKickInFlight || isPosterSetsWatcherBusy()) {
            return res.status(409).json({
                ...
                started: false,
                running: true,
                error: 'A watcher pass is already running. Check Logs shortly.',
            });
        }
        ...
        try {
            await appendPosterSetsAudit({
                ...
                state: 'running',
                detail: 'Check all started — scanning pinned sets in the background…',
                at: new Date().toISOString(),
            });
        } catch { /* ignore */ }
        ...
        res.json({
            ...
            started: true,
            running: true,
            message: 'Watcher pass started in the background. Results will appear in Logs.',
        });

        void (async () => {
            try {
                const result = await runPosterSetsWatcherPass({ forceAll: true, notify: true });
                ...
                try {
                    await appendPosterSetsAudit({
                        ...
                        state: errorCount ? 'partial' : (result.queued ? 'queued' : 'idle'),
                        error: errorCount
