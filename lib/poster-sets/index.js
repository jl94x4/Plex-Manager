...
/** Cap concurrent upstream image fetches so Watching grids don't stampede Node / Cloudflare. */
const IMAGE_PROXY_CONCURRENCY = 8;
...
export const createPosterSetsRouter = ({
            ...
            await withImageProxySlot(async () => {
                ...
                for (let attempt = 0; attempt < 2; attempt += 1) {
                    try {
                        ...
                        // Retry transient upstream pressure once.
                ...
                if (response.body && typeof Readable.fromWeb === 'function') {
                    try {
                        ...
                        if (!res.headersSent) sendPlaceholder(404);
                        return;
                    }
                }
    ...
    const tickPosterSetsQueue = async () => {
        ...
        try {
            ...
            // Keep worker alive across transient failures.
        } finally {
