            ...
            res.json({
                ...
                message: warmForcePublicSearch
                    ...
                    : hasTpdbCredentials(config)
                        ? `Queued ${queuedCount} library title(s) for metadata Warm${config.tpdbWarmParallelWorkers === true ? ' (5 parallel workers)' : ''} — set lists only; images hydrate on open.${skipNote}`
