    ...
    router.post('/search', async (req, res) => {
        try {
            ...
            const runCliSearch = async (providerId, extra = {}, { onBatch } = {}) => {
                const resolvedMedia = String(extra.mediaType ?? mediaType).trim().toLowerCase();
                const payload = {
                    provider: providerId,
                    query,
                    titleUrl: extra.titleUrl || titleUrl,
                    mediaType: resolvedMedia === 'tv' || resolvedMedia === 'show' ? 'show' : 'movie',
            ...
            if (titleSources?.length) {
                ...
                const settled = await Promise.allSettled(titleSources.map((source) => {
                    ...
                    const sourceMedia = String(
                        source?.mediaType || req.body?.mediaType || req.body?.media_type || 'movie',
                    ).trim().toLowerCase();
                    ...
                    return runCliSearch('posterdb', {
                        ...
                        tmdbId: linkedTmdb,
                        imdbId: source?.imdbId ?? source?.imdb_id ?? imdbId ?? null,
