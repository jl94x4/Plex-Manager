...
export const createPosterSetsRouter = ({
    ...
    router.post('/tpdb-cache/warm-library', async (req, res) => {
        try {
            ...
            const normalized = items.slice(0, 200).map((item) => ({
            ...
            if (!hasTpdbCredentials(config)) {
                logTpdbCacheActivity(
                    'Warm: TPDB username/password not configured — title resolve uses public text search only. Many titles may skip until you add login in Settings.',
                    { level: 'warn', current: true },
                );
            } else {
                logTpdbCacheActivity('Warm: using TPDB login for TMDB/IMDB/TVDB advanced resolve', {
                    current: true,
                });
                // Fail fast if credentials don't actually unlock advanced search.
                try {
                    ...
                    if (!loginProbe.ok) {
                        logTpdbCacheActivity(
                            `Warm: TPDB login probe failed — ${loginProbe.error || 'check username/password / Pro'}`,
                            { level: 'error', current: false },
                        );
                    } else {
                    ...
                    logTpdbCacheActivity(
                        `Warm: TPDB login probe error — ${probeError?.message || probeError}`,
                        { level: 'warn' },
                    );
                }
            }
            ...
            const queued = await enqueueTpdbLibraryWarm(normalized, async (item) => {
                ...
                if (!live.ok) {
                    ...
                    const logHint = (Array.isArray(live.logs) ? live.logs : [])
                        ...
                        .filter((line) => (
                            /login failed|credentials|redirected to login|session expired|lack Pro|no \/posters|<id> hits|advanced search found no|set load failed/i.test(line)
                            && !/^Searching ThePosterDB/i.test(line)
                        ))
                        .slice(-2)
                        .join(' | ');
                    // Soft-skip: title has no TPDB title page (or resolve/auth failed) — keep warming others.
                    if (/title url is required|could not (resolve|find).*title page|found no \/posters|needs a \/posters|worker timed out/i.test(errMsg)) {
                        warmSoftSkips += 1;
            ...
            res.json({
                ok: true,
                started: true,
                titles: queued.queued,
                message: hasTpdbCredentials(config)
                    ? `Queued ${queued.queued} library title(s) for rate-limited ThePosterDB warm (serial, 7s between requests).`
                    : `Queued ${queued.queued} title(s). Tip: add TPDB username/password in Settings so Warm can resolve by TMDB id (public search skips many titles).`,
            });
        } catch (error) {
