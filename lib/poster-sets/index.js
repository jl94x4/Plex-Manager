            ...
            const queued = await enqueueTpdbLibraryWarm(normalized, async (item) => {
                ...
                if (!live.ok) {
                    ...
                    const logHint = (Array.isArray(live.logs) ? live.logs : [])
                        ...
                        .filter((line) => (
                            /login failed|credentials|redirected to login|session expired|lack Pro|no \/posters|<id> hits|advanced search found no/i.test(line)
                            && !/^Searching ThePosterDB/i.test(line)
                        ))
                        .slice(-2)
                        .join(' | ');
                    // Soft-skip: title has no TPDB title page (or resolve/auth failed) — keep warming others.
                    if (/title url is required|could not (resolve|find).*title page|found no \/posters|needs a \/posters/i.test(errMsg)) {
                        return {
                            sets: [],
                            softSkip: true,
