...
import { coverageKeyForLibraryItem } from './tpdbCacheBrowse.js';
...
export const createPosterSetsRouter = ({
    ...
    const collectLibraryWarmItems = async ({
        ...
        const tryPush = (row) => {
            ...
            // Never treat media-server rating keys / Jellyfin GUIDs as TMDB ids.
            ...
            const key = coverageKeyForLibraryItem({ tmdbId, mediaType });
            if (!key || seen.has(key) || exhausted.has(key)) return false;
            if (skipCached && cachedKeys.has(key)) return false;
            seen.add(key);
            out.push({
                tmdbId,
        ...
        const hydrateMissingTmdb = async (rows = []) => {
            const list = Array.isArray(rows) ? rows : [];
            if (!list.length) return list;
            if (jellyfin) return list;
            const need = list.filter((row) => {
                const mediaType = resolvePosterSetsMediaType(row?.mediaType || row?.type, 'movie');
                if (resolvedWant && mediaType !== resolvedWant) return false;
                return !/^\d+$/.test(String(row?.tmdbId || '').trim()) && String(row?.id || '').trim();
            });
            if (!need.length) return list;
            const byId = new Map();
            const uniqueIds = [...new Set(need.map((row) => String(row.id)))];
            // Cap metadata lookups so warm collect stays responsive on huge libraries.
            const batch = uniqueIds.slice(0, 250);
            await Promise.all(batch.map(async (ratingKey) => {
                try {
                    const tmdbId = await fetchPlexItemTmdbId(portal, mediaLibraryDeps, ratingKey);
                    if (tmdbId) byId.set(ratingKey, String(tmdbId));
                } catch {
                    // Keep row without TMDB — push will skip it.
                }
            }));
            ...
            return list.map((row) => {
                ...
                return hydrated ? { ...row, tmdbId: hydrated } : row;
            });
        };
        ...
        if (source === 'recent' || source === 'full') {
            ...
            const recentRows = await hydrateMissingTmdb([
                ...(recent?.movies || []),
                ...(recent?.shows || []),
                ...(recent?.items || []),
            ]);
        ...
        if (source === 'full' && out.length < limit) {
            ...
            for (const section of sections || []) {
                ...
                const browsed = await hydrateMissingTmdb(browse?.items || []);
    ...
    router.post('/tpdb-cache/warm-library', async (req, res) => {
        try {
                ...
                if (fromLibrary) {
                    ...
                    if (!normalized.length) {
                        return res.json({
                            ...
                            titles: 0,
                            skippedCached: 0,
                            message: 'No uncached library titles with TMDB ids left to queue. Background catch-up will retry if new titles appear.',
                        });
                    }
                }
            }
            ...
            const queued = await enqueueTpdbLibraryWarm(normalized, async (item) => {
                ...
                warmSuccesses += 1;
                return result;
            }, { force, followedPrefetchOnly, followedCreatorsOnly, autoContinue: true, resetExhausted: fromLibrary || force });
