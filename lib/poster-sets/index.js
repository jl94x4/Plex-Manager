...
export const createPosterSetsRouter = ({
    ...
    router.post('/search', async (req, res) => {
        try {
            ...
            const runCliSearch = async (providerId, extra = {}) => {
                const payload = {
                    provider: providerId,
                    query,
            ...
            if (provider === 'both') {
                if (mode === 'creator') {
                    if (!query) return res.status(400).json({ error: 'creator username is required' });
                    ...
                    for (const result of settled) {
                        if (result.status === 'fulfilled') {
                            sets.push(...(result.value.sets || []));
                            logs.push(...(result.value.logs || []));
                            if (result.value.title) contextTitle = result.value.title;
                        } else {
