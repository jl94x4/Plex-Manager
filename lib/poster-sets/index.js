        ...
        try {
            ...
            res.json({
                ...
                message: hasTpdbCredentials(config)
                    ? `Queued ${queuedCount} library title(s) for rate-limited ThePosterDB warm (serial, 7s between requests).${skipNote}`
                    : `Queued ${queuedCount} title(s).${skipNote} Tip: add TPDB username/password in Settings so Warm can resolve by TMDB id (public search skips many titles).`,
            });
        } catch (error) {
