    ...
    router.post('/tpdb-cache/warm-library', async (req, res) => {
        try {
            ...
            const queued = await enqueueTpdbLibraryWarm(normalized, async (item) => {
                ...
                if (!live.ok) {
                    throw new Error(live.error || 'ThePosterDB search failed');
                }
                return live.result || { sets: [] };
            });
            ...
            res.json({
                ...
                message: `Queued ${queued.queued} library title(s) for rate-limited ThePosterDB warm (serial resolves, ≤${22} asset downloads/min).`,
            });
        } catch (error) {
