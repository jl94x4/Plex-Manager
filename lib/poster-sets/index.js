        ...
        try {
            ...
            const runCliSearch = async (providerId, extra = {}, { onBatch } = {}) => {
                ...
                const payload = {
                    ...
                    titleHint: extra.titleHint ?? req.body?.title ?? query ?? null,
                    yearHint: extra.yearHint ?? req.body?.year ?? null,
                    mode,
                    limit,
                    batchPages,
            ...
            if (titleSources?.length) {
                const settled = await Promise.allSettled(titleSources.map((source) => {
                    ...
                    return runCliSearch('posterdb', {
                        ...
                        tmdbId: null,
                        imdbId: source?.imdbId ?? source?.imdb_id ?? null,
                        titleHint: req.body?.title || query || null,
                        yearHint: source?.year ?? null,
                    });
                }));
                const sets = [];
                const logs = [];
