        ...
        try {
            ...
            if (titleSources?.length) {
                ...
                const settled = await Promise.allSettled(titleSources.map(async (source) => {
                    ...
                    // Library SWR: return TPDB cache immediately when live would block; still refresh below.
                    if (cachedTitle && !titleCacheNeedsRevalidate(cachedTitle)) {
                        logTpdbCacheActivity(
                    ...
                    try {
                        ...
                        if (cachedTitle) {
                            return {
                                ...
                                logs: Array.isArray(error?.logs) ? error.logs : [],
                            };
                        }
                        throw error;
                    }
                ...
                // Soft-revalidate stale library cache in background.
                if (cachedTitle && titleCacheNeedsRevalidate(cachedTitle) && libraryScoped && cacheEnabled) {
                    void (async () => {
                        try {
                            const externalIds = await externalIdsPromise;
                            const live = await runCliSearch('posterdb', {
        ...
        try {
            ...
            const queued = await enqueueTpdbLibraryWarm(normalized, async (item) => {
                ...
                if (!live.ok) {
                    ...
                    // Soft-skip: title has no TPDB /posters/ page (or resolve failed) — keep warming others.
                    if (/title url is required|could not resolve|found no \/posters/i.test(errMsg)) {
                        return {
                            sets: [],
                            softSkip: true,
                            softError: errMsg,
