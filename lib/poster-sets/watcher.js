 ...
 * - brand new IDs on the set (creator uploaded more art), and/or
 * - assets that now match the library but were never successfully applied
 *   (e.g. Season 4 cover was on TPDB at pin time, season only just landed on Plex)
 * - when seasonNumber is set (Sonarr), force that season's art even if previously "known"
 */
export const pickWatchApplyIds = (assets, {
    ...
    if (isFirstCheck) {
        return {
            ...
            newIds: [],
            pendingIds: [],
            knownAssetIds: list.map((asset) => String(asset?.id || '').trim()).filter(Boolean),
            appliedAssetIds: list
    ...
    for (const asset of list) {
        ...
        if (seasonNumber != null && Number.isFinite(Number(seasonNumber))) {
            if (assetTargetsSeason(asset, seasonNumber)) pendingIds.push(id);
            continue;
        }

        // Season/show art that Plex can accept now (matched) but never uploaded.
        if (asset?.matched === true) pendingIds.push(id);
    }
...
export const checkPosterSetsWatch = async (watch, { enqueue = true, seasonNumber = null } = {}) => {
    ...
    if (enqueue && queueIds.length) {
        const jobInput = {
            ...
            ...(Number.isFinite(Number(seasonNumber)) ? { seasonNumber: Number(seasonNumber) } : {}),
        };
        if (enqueueApplyFn) {
    ...
    // Fingerprints / applied update on successful apply (via queue completion hook).
    // Track check metadata now; keep known/applied unchanged until apply succeeds —
    // except promote brand-new IDs into known only after apply. Keep known stable here.
    const patched = await patchPosterSetsWatch(watch.id, {
        title: setMeta.title || watch.title,
        user: setMeta.user || watch.user,
        thumbUrl: setMeta.thumbUrl || watch.thumbUrl || '',
