...
export const checkPosterSetsWatch = async (watch, { enqueue = true, seasonNumber = null } = {}) => {
    ...
    // One-time quiet heal: stop full-set / season-cover storms and seed lastMatched.
    const needsQuietBaseline = !isFirstCheck && (
        trackingVersion < WATCH_ASSET_TRACKING_VERSION
        || lastMatchedList.length === 0
        || (appliedList.length === 0 && knownList.length > 0)
    );
    ...
    if (isFirstCheck || needsQuietBaseline) {
        ...
        const patched = await patchPosterSetsWatch(watch.id, {
            ...
            appliedAssetIds: picked.appliedAssetIds || matchedIds,
