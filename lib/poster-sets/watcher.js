 ...
 * - brand-new IDs on the set (creator uploaded more art)
 * - Sonarr season import: force that season's covers/title cards
 * - library-matched assets not yet marked applied (season landed on Plex later)
 *
 * Baseline / migrate must set applied = currently matched only — never the full
 * known set — or season covers that appear later on Plex never queue.
 */
export const pickWatchApplyIds = (assets, {
    ...
    const applied = new Set(asIdList(appliedAssetIds));
    ...
    if (isFirstCheck) {
        // Seen on the set ≠ applied. Only mark what already matches in the library.
        return {
            queueIds: [],
            newIds: [],
            pendingIds: [],
    ...
    for (const asset of list) {
        ...
        // ARR import for a specific season — re-apply that season's art even if previously known.
        if (forceSeason && wantSeason != null && assetTargetsSeason(asset, wantSeason)) {
            pendingIds.push(id);
            continue;
        }

        // Season (or other art) now matches in Plex but was never uploaded.
        if (!applied.has(id) && asset?.matched === true) {
            pendingIds.push(id);
        }
    }
...
const isSeasonCoverAsset = (asset) => {
    const filter = String(asset?.fileType || asset?.file_type || '').trim();
    if (filter === 'season_cover') return true;
    if (filter) return false;
    if (String(asset?.kind || '') !== 'show') return false;
    const season = asset?.season;
    const episode = asset?.episode;
    if (season === 'Cover' || season === 'Backdrop') return false;
    if (episode === 'Cover' || episode == null || episode === '') {
        return assetSeasonNumber(asset) != null;
    }
    return false;
};

/** True when applied was baselined as the full set (blocks season-later applies). */
const appliedLooksLikeFullSetBaseline = (knownList, appliedList) => {
    if (!knownList.length || !appliedList.length) return false;
    if (appliedList.length < knownList.length) return false;
    const applied = new Set(appliedList);
    return knownList.every((id) => applied.has(id));
};
...
export const checkPosterSetsWatch = async (watch, { enqueue = true, seasonNumber = null } = {}) => {
    ...
    const assetById = new Map(assets.map((asset) => [String(asset?.id || '').trim(), asset]));
    const knownList = asIdList(watch.knownAssetIds);
    const appliedList = asIdList(watch.appliedAssetIds);
    ...
    // Legacy: known set but empty applied — seed from current library matches only.
    const needsAppliedMigrate = !isFirstCheck && appliedList.length === 0 && knownList.length > 0;
    // Storm-fix migrate marked applied = known, so season covers never queued later.
    const needsSeasonLaterHeal = !isFirstCheck
        && trackingVersion < 2
        && appliedLooksLikeFullSetBaseline(knownList, appliedList);

    let effectiveApplied = appliedList;
    if (needsAppliedMigrate) {
        effectiveApplied = matchedIds;
    } else if (needsSeasonLaterHeal) {
        // Drop season covers from the poisoned full-set baseline so matched seasons can queue.
        effectiveApplied = appliedList.filter((id) => !isSeasonCoverAsset(assetById.get(id)));
    }

    const picked = pickWatchApplyIds(assets, {
        knownAssetIds: knownList,
        appliedAssetIds: effectiveApplied,
    ...
    if (isFirstCheck) {
        const patched = await patchPosterSetsWatch(watch.id, {
            knownAssetIds: picked.knownAssetIds,
            appliedAssetIds: picked.appliedAssetIds,
            assetTrackingVersion: 2,
    ...
    const patched = await patchPosterSetsWatch(watch.id, {
        ...
        ...(needsAppliedMigrate || needsSeasonLaterHeal
            ? { appliedAssetIds: effectiveApplied }
            : {}),
        assetTrackingVersion: 2,
        ...idPatch,
    ...
    return {
        ...
        baseline: false,
        setMeta,
        migratedApplied: needsAppliedMigrate,
        healedSeasonLater: needsSeasonLaterHeal,
    };
};
...
export const autoWatchFromApply = async ({ url, setMeta, selectedIds, mediuxFilters, plexHint } = {}) => {
    ...
    if (!watchFilters) {
        ...
        } else if (appliedSelected.length) {
            try {
                const inspected = await inspectSetAssets(target, config.mediux_filters);
                const derived = filtersFromSelectedAssets(inspected.assets, appliedSelected);
