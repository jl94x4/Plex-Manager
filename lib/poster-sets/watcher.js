 ...
 *
 * Do NOT re-queue every library-matched asset on Check all — legacy watches have
 * empty appliedAssetIds, which previously caused a mass re-apply storm.
 */
export const pickWatchApplyIds = (assets, {
    ...
    if (isFirstCheck) {
        ...
        // Baseline: everything currently on the set is "seen" and treated as satisfied.
        // Future creator uploads (new IDs) and Sonarr season forces still apply later.
        return {
            queueIds: [],
            newIds: [],
            pendingIds: [],
            knownAssetIds: allIds,
            appliedAssetIds: allIds,
        };
    }
    ...
    for (const asset of list) {
        ...
        // Cautious pending path: only when we have real applied tracking and this
        // asset is matched in Plex but never successfully uploaded.
        if (applied.size > 0 && !applied.has(id) && asset?.matched === true) {
            pendingIds.push(id);
        }
    }
...
export const checkPosterSetsWatch = async (watch, { enqueue = true, seasonNumber = null } = {}) => {
    ...
    const wantSeason = seasonNumber != null && Number.isFinite(Number(seasonNumber))
        ? Number(seasonNumber)
        : null;
    // Legacy watches (known set, empty applied) must not treat every match as pending.
    const needsAppliedMigrate = !isFirstCheck && appliedList.length === 0 && knownList.length > 0;
    const effectiveApplied = needsAppliedMigrate ? knownList : appliedList;
    ...
    const patched = await patchPosterSetsWatch(watch.id, {
        ...
        thumbUrl: setMeta.thumbUrl || watch.thumbUrl || '',
        setId: setMeta.setId || watch.setId,
        // One-time migrate: seed applied from known so Check all never storms again.
        ...(needsAppliedMigrate ? { appliedAssetIds: knownList } : {}),
        ...idPatch,
        lastCheckedAt: new Date().toISOString(),
        lastError: null,
