...
const normalizeCreatorHandle = (value) => String(value || '').trim().replace(/^@+/, '').toLowerCase();
...
export const pickMediuxTmdbForHint = (
    ...
    for (const title of titles) {
        ...
        if (wantYear != null) {
            ...
            else if (year != null && Math.abs(year - wantYear) === 1) score += 15;
            else score -= 40;
        }
        const mt = String(title.mediaType || '').trim().toLowerCase();
        if (wantMedia === 'show' || wantMedia === 'tv' || wantMedia === 'series') {
            score += mt === 'show' ? 8 : -20;
...
export const mergePosterSearchSets = (sets = [], preferred = 'posterdb', options = {}) => {
    ...
    return {
        sets: prioritizeSetsByFollowedCreators(merged, preferredCreators),
        dupesCollapsed,
    };
};
