...
export const resolvePosterSetsTitleStatus = async ({
    ...
    const matchingWatches = findWatchesForLibraryTitle(watches, { ratingKey: key, title: needle });
    const titleMatchedWatches = matchingWatches.length
        ? matchingWatches
        : watches.filter((watch) => needle && watch.title && titleMatches(watch.title, needle));

    const activeWatches = titleMatchedWatches.filter((watch) => watch.enabled !== false);
    const primaryWatch = resolvePrimaryTitleWatch(watches, {
        ratingKey: key,
        title: needle,
