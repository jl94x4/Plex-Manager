...
import { normalizeCreatorHandle, keepFollowedCreatorsOnly, prioritizeSetsByFollowedCreators } from './searchMerge.js';
import {
...
export const listTpdbCachedCoverageKeys = async ({ force = false } = {}) => {
    ...
    await Promise.all(names.map(async (name) => {
        const key = coverageKeyFromTitleCacheFileName(name);
        if (!key) return;
        const entry = await readJsonFile(path.join(TPDB_TITLE_CACHE_DIR, name));
        if (entry && Array.isArray(entry.sets) && entry.sets.length) keys.add(key);
    }));
...
const rememberExhaustedCoverageKey = (item) => {
    ...
    if (key) exhaustedCoverageKeys.add(key);
};

const forgetExhaustedCoverageKey = (item) => {
    const key = coverageKeyForLibraryItem(item);
    if (key) exhaustedCoverageKeys.delete(key);
};
...
export const priorityEnsureTpdbTitleCached = async (item = {}, options = {}) => {
    ...
    if (options.force !== true) {
        const existing = await loadTpdbTitleCache(titleKey);
    ...
    const matchesQueueItem = (entry) => {
        ...
        return String(row?.tmdbId || '').trim() === tmdbId
            && (String(row?.mediaType || 'movie').toLowerCase() === 'show' ? 'show' : 'movie') === mediaType;
    };
...
const hydrateOneSet = async (item) => {
    ...
    if (item?.user && blocked.some((name) => normalizeCreatorHandle(name) === normalizeCreatorHandle(item.user))) {
        return;
    }
    const exclusive = hydrateFollowedExclusiveSession || config.tpdbCacheFollowedCreatorsOnly === true;
    if (exclusive) {
...
export const enqueueTpdbLibraryTitleHydrate = async (sets = [], options = {}) => {
    ...
    if (!options.libraryScoped && !options.tmdbId && !options.collectionSets) return { queued: 0 };
...
export const enqueueTpdbLibraryWarm = async (items = [], fetchTitleSets, options = {}) => {
    ...
    for (const item of (Array.isArray(items) ? items : []).slice(0, WARM_QUEUE_MAX)) {
        ...
        const coverageKey = coverageKeyForLibraryItem({ tmdbId, mediaType });
        if (!force && coverageKey && exhaustedCoverageKeys.has(coverageKey)) continue;
        if (!force && titleKey) {
            const cached = await loadTpdbTitleCache(titleKey);
            if (cached?.sets?.length) {
                skippedCached += 1;
...
export const resumeTpdbWarmQueueFromDisk = async () => {
    ...
    if (Array.isArray(progress?.exhaustedKeys)) {
        exhaustedCoverageKeys.clear();
        for (const key of progress.exhaustedKeys) {
            const value = String(key || '').trim();
            if (value) exhaustedCoverageKeys.add(value);
        }
    }
    ...
    try {
        ...
        if (useBatch && chunkItems.length) {
            ...
            for (const entry of chunkEntries) {
                ...
                const key = String(item?.tmdbId || item?.id || '').trim();
                if (key && appliedKeys.has(key)) continue;
                const result = key ? byTmdb.get(key) : null;
                if (result) {
        ...
        if (loginBroken) {
            for (const entry of chunkEntries) {
                const key = String(entry?.item?.tmdbId || entry?.item?.id || '').trim();
                if (key && appliedKeys.has(key)) continue;
