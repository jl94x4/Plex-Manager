...
export const resumeTpdbWarmQueueFromDisk = async () => {
    ...
    if (typeof warmFetchTitleSetsFn !== 'function') return { resumed: 0 };
    ...
    const result = await enqueueTpdbLibraryWarm(pending, warmFetchTitleSetsFn, { force: false });
...
const pumpWarmTitleQueue = async () => {
    ...
    try {
        ...
        if (/title url is required|could not (resolve|find).*title page|found no \/posters|needs a \/posters/i.test(message)) {
