...
export const enqueueTpdbLibraryTitleHydrate = async (sets = [], options = {}) => {
    ...
    const prioritizeFollowed = config.tpdbPrioritizeFollowedCreators !== false && whitelist.length > 0;
    ...
    for (const set of list) {
        ...
        const item = {
            ...
            user,
            preferred: prioritizeFollowed && user
    ...
    if (preferredItems.length) {
        // Jump ahead of already-queued non-followed sets while preserving whitelist order.
    ...
    for (const item of (Array.isArray(items) ? items : []).slice(0, WARM_QUEUE_MAX)) {
        ...
        if (!force && titleKey) {
            ...
            if (cached?.sets?.length) {
                skippedCached += 1;
                // Title metadata already on disk — still queue set/image hydrate when Prefetch is on.
...
const pumpWarmTitleQueue = async () => {
    ...
    try {
        ...
        if (useBatch && chunkItems.length) {
            ...
            if (workerCount <= 1) {
                logTpdbCacheActivity(
                    ...
                    { current: `Cache batch · ${chunkItems.length} title(s)` },
                );
                ...
                logTpdbCacheActivity(
                    ...
                    { current: `Cache ×${activeShards.length} · ${chunkItems.length} title(s)` },
                );
        ...
        if (/title url is required|could not (resolve|find).*title page|found no \/posters|needs a \/posters|login failed/i.test(message)) {
            logTpdbCacheActivity(`Cache: skipped — ${message}`, { level: 'warn' });
        } else {
            hydrateLastError = message;
            logTpdbCacheActivity(`Cache error: ${hydrateLastError}`, { level: 'error', current: false });
        }
