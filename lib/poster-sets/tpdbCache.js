...
/**
 * Re-scrape every cached library title once and merge any new TPDB sets into the permanent local DB.
 */
export const runTpdbCacheDailyRefreshPass = async () => {
    ...
    if (warmTitleActive > 0 || warmTitleQueue.length > 0) {
        logTpdbCacheActivity('Daily refresh deferred — library cache build still running', {
            level: 'warn',
            kind: 'cache',
        });
        return { skipped: true, reason: 'warm-busy' };
    }
    if (typeof warmFetchTitleSetsFn !== 'function' && typeof warmBatchRunnerFn !== 'function') {
    ...
    logTpdbCacheActivity('Daily refresh: checking cached titles for new poster sets…', {
        kind: 'cache',
        current: 'Daily TPDB cache refresh…',
    });
    ...
    try {
        ...
        if (!titles) {
            ...
            logTpdbCacheActivity('Daily refresh: no cached titles yet', { kind: 'cache', current: false });
            return empty;
        }

        const handleResult = async (item, result) => {
            try {
                ...
                if (outcome.added > 0) {
                    logTpdbCacheActivity(
                        `Daily refresh: ${item.title} +${outcome.added} new set(s)`,
                ...
                failed += 1;
                logTpdbCacheActivity(
                    `Daily refresh failed for ${item.title}: ${error?.message || error}`,
                    { level: 'warn', kind: 'error', detail: `tmdb ${item.tmdbId}` },
                );
            }
        };

        if (typeof warmBatchRunnerFn === 'function') {
            for (let i = 0; i < items.length; i += WARM_BATCH_SIZE) {
                if (cacheWorkPaused) break;
                const chunk = items.slice(i, i + WARM_BATCH_SIZE);
                const byTmdb = new Map(chunk.map((row) => [String(row.tmdbId), row]));
                await warmBatchRunnerFn(chunk, {
                    onTitleResult: async (row, result) => {
                        const item = byTmdb.get(String(row?.tmdbId || '')) || row;
                        await handleResult({ ...item, existingSets: item.existingSets || [] }, result);
                    },
                });
            }
        } else {
            for (const item of items) {
                if (cacheWorkPaused) break;
                await waitForTpdbRequestSlot();
                const result = await warmFetchTitleSetsFn(item);
                await handleResult(item, result);
                await sleep(BETWEEN_WARM_TITLES_MS);
            }
        }

        const summary = { refreshed, addedSets, failed, titles };
        dailyRefreshStatus.lastResult = summary;
        logTpdbCacheActivity(
            `Daily refresh done — ${refreshed}/${titles} titles checked, +${addedSets} new set(s)${failed ? `, ${failed} error(s)` : ''}`,
            { kind: 'cache', current: false },
        );
        return summary;
    } catch (error) {
        ...
        logTpdbCacheActivity(`Daily refresh aborted: ${message}`, { level: 'error', kind: 'error', current: false });
...
const armTpdbDailyRefreshTimer = async () => {
    ...
    dailyRefreshTimer = setTimeout(() => {
        void (async () => {
            try {
                const result = await runTpdbCacheDailyRefreshPass();
                // If a library warm was busy, retry in an hour instead of waiting for the next slot.
                if (result?.skipped && result.reason === 'warm-busy') {
                    dailyRefreshTimer = setTimeout(() => {
                        void runTpdbCacheDailyRefreshPass().finally(() => { void armTpdbDailyRefreshTimer(); });
                    }, 60 * 60_000);
                    dailyRefreshTimer.unref?.();
                    dailyRefreshStatus.nextRunAt = new Date(Date.now() + 60 * 60_000).toISOString();
                    return;
                }
            } catch {
                /* keep schedule alive */
