...
/** Minimum spacing between any TPDB asset HTTP requests (images). */
const IMAGE_MIN_GAP_MS = 2500;
/** Extra pause after each set-preview CLI scrape before images begin. */
const PREVIEW_SCRAPE_GAP_MS = 4000;
/** Extra pause after finishing a hydrated set before the next set starts. */
const BETWEEN_SETS_GAP_MS = 5000;
/**
 * ThePosterDB asset API is roughly ~30 req / window.
 * Stay under that with a rolling 60s budget shared by all hydrate downloads.
 */
const TPDB_ASSET_WINDOW_MS = 60_000;
const TPDB_ASSET_MAX_PER_WINDOW = 22;
...
/** @type {number[]} timestamps of recent asset download attempts */
const assetRequestTimes = [];
let lastAssetRequestAt = 0;
let assetGate = Promise.resolve();
let rateLimitCooldownUntil = 0;

const pruneAssetWindow = (now = nowMs()) => {
    while (assetRequestTimes.length && (now - assetRequestTimes[0]) > TPDB_ASSET_WINDOW_MS) {
        assetRequestTimes.shift();
    }
};

/**
 * Global polite gate for TPDB asset downloads (shared across hydrate + warm).
 * Enforces min gap + rolling window cap + cooldown after HTTP 429.
 */
export const waitForTpdbAssetSlot = async () => {
    const run = assetGate.then(async () => {
        for (;;) {
            const now = nowMs();
            ...
            pruneAssetWindow(now);
            const sinceLast = now - lastAssetRequestAt;
            if (lastAssetRequestAt && sinceLast < IMAGE_MIN_GAP_MS) {
                await sleep(IMAGE_MIN_GAP_MS - sinceLast);
                continue;
            }
            if (assetRequestTimes.length >= TPDB_ASSET_MAX_PER_WINDOW) {
                const wait = TPDB_ASSET_WINDOW_MS - (now - assetRequestTimes[0]) + 250;
                await sleep(Math.max(500, wait));
                continue;
            }
            lastAssetRequestAt = nowMs();
            assetRequestTimes.push(lastAssetRequestAt);
            return;
        }
    });
    assetGate = run.catch(() => {});
...
const noteTpdbRateLimit = (retryAfterHeader = null) => {
    ...
    const waitMs = Number.isFinite(retryAfterSec) && retryAfterSec > 0
        ? Math.min(120_000, Math.max(15_000, retryAfterSec * 1000))
        : 45_000;
...
export const getTpdbCacheStatus = async () => {
    ...
    return {
        ...
        hydrate: {
            ...
            rateLimit: {
                assetRequestsInWindow: (() => {
                    pruneAssetWindow();
                    return assetRequestTimes.length;
                })(),
                maxPerWindow: TPDB_ASSET_MAX_PER_WINDOW,
                windowMs: TPDB_ASSET_WINDOW_MS,
...
const BETWEEN_WARM_TITLES_MS = 8_000;
...
const hydrateOneSet = async (item) => {
    ...
    if (config.tpdbLocalCacheEnabled === false) return;
    ...
    if (!assets.length) {
        ...
        // Space HTML scrapes away from the following asset burst.
        await sleep(PREVIEW_SCRAPE_GAP_MS);
    }
...
export const enqueueTpdbLibraryTitleHydrate = async (sets = [], options = {}) => {
    const config = await loadPosterSetsConfig();
    if (config.tpdbLocalCacheEnabled === false) return { queued: 0 };
    if (config.tpdbAggressivePrefetch === false && !options.force) return { queued: 0 };
