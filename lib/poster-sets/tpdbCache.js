    ...
    for (const item of (Array.isArray(items) ? items : []).slice(0, WARM_QUEUE_MAX)) {
        ...
        if (!force && titleKey) {
            ...
            if (cached?.sets?.length) {
                skippedCached += 1;
                // Metadata-only Warm — image hydrate happens on title open / prefetch.
                continue;
            }
        }
...
export const applyTpdbWarmTitleResult = async (item, result) => {
    ...
    if (sets.length) {
        ...
        // Metadata-only: Warm caches title + set list. Image/set HTML hydrate runs when
        // the user opens a title (or has prefetch on for interactive loads) — not here.
        return;
    }
...
const pumpWarmTitleQueue = async () => {
    ...
    try {
        ...
        if (useBatch && chunkItems.length) {
            ...
            if (workerCount <= 1) {
                logTpdbCacheActivity(
                    `Warm batch: resolving ${chunkItems.length} title(s) (metadata only)`,
                ...
                logTpdbCacheActivity(
                    `Warm parallel: ${chunkItems.length} title(s) across ${activeShards.length} workers (metadata only)`,
                    { current: `Warm ×${activeShards.length} · ${chunkItems.length} title(s)` },
                );
