...
/** When true, Prefetch only enqueues Creators you follow sets. */
...
export const enqueueTpdbLibraryTitleHydrate = async (sets = [], options = {}) => {
    ...
    const followedOnly = options.followedOnly === true || hydrateFollowedOnlySession === true;
    let list = Array.isArray(sets) ? [...sets] : [];
    if (prioritizeFollowed || followedOnly) {
    ...
    for (const set of list) {
        ...
        const preferred = (prioritizeFollowed || followedOnly) && user
            ? followedKeys.has(normalizeCreatorHandle(user))
            : false;
        if (followedOnly && !preferred) continue;
    ...
    if (queued > 0) {
        ...
        logTpdbCacheActivity(
            preferredQueued > 0
                ...
                : followedOnly
                    ? `Queued ${queued} followed-creator set(s) for prefetch`
                    : `Queued ${queued} set(s) for image/prefetch hydrate`,
            {
                kind: preferredQueued || followedOnly ? 'followed' : 'prefetch',
