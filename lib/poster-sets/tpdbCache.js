...
export const warmLibraryItemKey = (item = {}) => {
    const tmdbId = String(item?.tmdbId || '').trim();
    if (/^\d+$/.test(tmdbId)) return `tmdb:${tmdbId}`;
    const tvdbId = String(item?.tvdbId || '').trim();
    if (/^\d+$/.test(tvdbId)) return `tvdb:${tvdbId}`;
    return '';
};
...
const requeueUnappliedChunk = (chunkEntries, appliedKeys, reason) => {
    ...
    for (const entry of chunkEntries) {
        ...
        const key = warmLibraryItemKey(item);
    ...
    try {
        ...
        if (useBatch && chunkItems.length) {
            ...
            for (const row of (Array.isArray(results) ? results : [])) {
                const key = warmLibraryItemKey(row);
        ...
        if (loginBroken) {
            for (const entry of chunkEntries) {
                const key = warmLibraryItemKey(entry?.item);
                if (key && appliedKeys.has(key)) continue;
