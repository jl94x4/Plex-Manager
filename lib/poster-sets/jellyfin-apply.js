...
const pickBestItem = (items, { year, type }) => {
    const list = Array.isArray(items) ? items : [];
    if (!list.length) return null;
    const wantedYear = Number(year);
    if (Number.isFinite(wantedYear)) {
        const byYear = list.find((item) => Number(item.ProductionYear) === wantedYear);
        if (byYear) return byYear;
    }
    if (type === 'movie') {
        return list.find((item) => item.Type === 'Movie') || list[0];
    }
    if (type === 'show') {
        return list.find((item) => item.Type === 'Series') || list[0];
    }
    return list[0];
};
...
const findSeriesItem = async (ctx, asset) => {
    ...
    return pickBestItem(items, { year: asset.year, type: 'show' });
};
...
const findMovieItem = async (ctx, asset) => {
    ...
    return pickBestItem(items, { year: asset.year, type: 'movie' });
};
...
export const applyPosterSetToJellyfinLike = async ({
    ...
    return {
        ok: uploaded > 0 || results.every((entry) => entry.ok),
        url: targetUrl,
        uploaded,
