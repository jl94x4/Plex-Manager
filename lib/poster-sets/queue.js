...
/** Cancel every waiting queue item (keeps running/finished). */
    ...
    const state = await updatePosterSetsQueue((current) => {
        ...
        current.jobs = current.jobs.map((job) => {
            ...
            return {
                ...
                state: 'cancelled',
                finishedAt: now,
                error: 'Cancelled while queued (clear queued)',
            };
        });
        return current;
    });
    return { state, cancelled: cancelledIds.length, cancelledIds };
};
