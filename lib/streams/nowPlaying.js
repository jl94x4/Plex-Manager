...
const isActivePlexSession = (metadata = {}) => {
    const state = String(sessionPlayer(metadata).state || 'playing').toLowerCase();
    return state === 'playing' || state === 'buffering';
};
 ...
 * fall back to owner account "1", then to the sole playable session on quiet servers.
 */
export const pickOwnPlexNowPlayingSession = (sessions, identity = {}) => {
    ...
    if (!own.length && identity?.isAdmin) {
        own = list.filter((metadata) => sessionBelongsToPlexUser(metadata, {
            ...identity,
            accountId: '1',
    ...
    if (!own.length && identity?.isAdmin) {
        const playable = list.filter(isPlayablePlexNowPlayingType);
        const activePlayable = playable.filter(isActivePlexSession);
        if (activePlayable.length === 1) own = activePlayable;
        else if (playable.length === 1) own = playable;
    }
