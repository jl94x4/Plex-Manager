...
import { fetchMusicBrainzArtist } from '../musicbrainz-client.js';
...
export const createPortalRequestService = ({
    ...
    const createMemberRequest = async (user, body = {}) => {
        ...
        if (duplicate) {
            const err = new Error(
                mediaType === 'tv'
                    ...
                    : (mediaType === 'music'
                        ? 'You already requested this artist.'
        ...
        if (mediaType === 'music') {
            ...
            meta = {
                ...
                genres: artist?.tags || [],
                originalLanguage: null,
                posterPath: artist?.posterUrl || artist?.posterPath || null,
                backdropPath: null,
            };
        } else {
     ...
     */
    const getMemberMusicRequestOptions = async (user, { mbid } = {}) => {
        ...
        if (portalRequestRows.some((row) => Number(row.status) === REQUEST_STATUS_PENDING)) {
            mediaStatus = 2;
            canRequest = false;
            blockReason = 'This artist is already requested.';
        ...
        const title = details?.name || details?.title || 'Unknown artist';
        const posterUrl = libraryStatus?.posterUrl || details?.posterUrl || details?.posterPath || null;
    ...
    const getMemberRequestOptions = async (user, { mediaType, mediaId } = {}) => {
        ...
        if (rawType === 'music') {
            return getMemberMusicRequestOptions(user, { mbid: mediaId });
        }
