...
export const createPortalRequestService = ({
    ...
    const getMemberMusicRequestOptions = async (user, { mbid } = {}) => {
        ...
        if (canRequest && servers[0]) {
            const instance = resolvePortalArrInstance(config, 'music', servers[0].id, servers[0].instanceId);
            if (instance) {
                const inLibrary = await findLidarrArtistInLibrary(instance, artistId, fetchOpts).catch(() => null);
                if (inLibrary?.id) {
                    canRequest = false;
                    mediaStatus = 5;
                    blockReason = 'This artist is already in your library.';
                }
            }
        }
        ...
        const posterUrl = details?.posterUrl || details?.posterPath || null;
        ...
        return {
            ...
            blockReason,
            seasons: [],
            mediaInfo: { requests: portalRequestRows, status: mediaStatus },
        };
    };
