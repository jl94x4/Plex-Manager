...
export const createPortalRequestService = ({
    ...
    /** Stay under reverse-proxy idle limits (often 60s) so Save & Approve never blank-502s. */
    const APPROVE_PUSH_BUDGET_MS = 12_000;
    ...
    const markRequestFailedFromPush = async (requestId, record, pushResult, adminUser) => (
        store.update(requestId, {
            ...
            meta: {
                ...
                modifiedBy: modifiedByMeta(adminUser, record.meta?.modifiedBy || null),
            },
        })
    );

    const markRequestApprovedFromPush = async (requestId, record, pushResult, adminUser, extraMeta = {}) => (
        store.update(requestId, {
    ...
    const approveAdminRequest = async (requestId, overrides = null, adminUser = null) => {
        ...
        // Sonarr lookup/POST (especially multi-season TV) can exceed nginx/Cloudflare idle
        // timeouts and surface as a blank 502. Wait briefly; if still going, return approved
        // as processing and finish the *arr push in the background.
        const pushPromise = pushRecordToArr(record);
        const raced = await Promise.race([
            pushPromise.then((pushResult) => ({ settled: true, pushResult })),
            new Promise((resolve) => {
                setTimeout(() => resolve({ settled: false }), APPROVE_PUSH_BUDGET_MS);
            }),
        ]);

        if (raced.settled) {
            const { pushResult } = raced;
            if (!pushResult.ok) {
                const failed = await markRequestFailedFromPush(requestId, record, pushResult, adminUser);
                const err = new Error(pushResult.reason || 'Failed to push to *arr');
                err.status = 502;
                err.request = await enrichDto(failed, { detail: true });
                throw err;
            }
            const approved = await markRequestApprovedFromPush(requestId, record, pushResult, adminUser);
            return enrichDto(approved, { detail: false });
        }

        const pendingApproved = await store.update(requestId, {
            status: REQUEST_STATUS_APPROVED,
            meta: {
                ...(record.meta || {}),
                arrError: null,
        ...
        void pushPromise.then(async (pushResult) => {
            try {
                if (!pushResult.ok) {
                    await markRequestFailedFromPush(requestId, record, pushResult, adminUser);
                    return;
                }
                await markRequestApprovedFromPush(requestId, record, pushResult, adminUser);
            } catch {
                // Background *arr sync is best-effort; retry can recover.
            }
        });

        return enrichDto(pendingApproved, { detail: false });
    };
