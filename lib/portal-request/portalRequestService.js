...
export const createPortalRequestService = ({
    ...
    const declineAdminRequest = async (requestId, reason = '', adminUser = null) => {
        ...
        if (Number(record.status) === REQUEST_STATUS_APPROVED && record.meta?.arrPushPending) {
            // Allow decline to win over an in-flight *arr push; generation checks discard the push result.
        } else if (Number(record.status) === REQUEST_STATUS_DECLINED) {
            return enrichDto(record, { detail: true });
        }
