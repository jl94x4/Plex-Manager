...
const coerceTvPartialEntry = (entry) => {
    ...
    const hasOnDisk = episodeFiles > 0 || totalAired > 0 || Boolean(sonarr?.showComplete);
    // Snapshot rows omit sonarr.seasons — do not treat missing aired totals as "unreleased".
    const stripAvailable = !hasOnDisk
        && (status === SEERR_MEDIA_AVAILABLE || status === SEERR_MEDIA_PARTIAL)
        && Boolean(sonarr?.nextAiring);
    const stripPrePremiere = !hasOnDisk
        && (status === SEERR_MEDIA_AVAILABLE || status === SEERR_MEDIA_PARTIAL)
        && !sonarr?.nextAiring;
    ...
    if (!stripAvailable && !stripPrePremiere) return entry;
    return {
        ...entry,
        mediaInfo: {
            ...entry.mediaInfo,
