...
export const createRequestAppService = ({
    ...
    const getMemberRequestOptions = async (config, sessionUser, { mediaType, mediaId }) => {
        ...
        // Portal always exposes the full request form when Arr servers exist.
        // Member submits use a Plex cookie session (not the admin API key).
        ...
        permissionFlags.requestTags = config.portalAllowRequestTags !== false;
        ...
        return {
            ...
            canRequest4k: canRequest4k && has4kServer,
            canRequestAdvanced,
            canRequestTags: config.portalAllowRequestTags !== false,
            canCreateIssues,
            has4kServer,
            hasHdServer,
            ...
            permissions: {
                ...
                requestTags: config.portalAllowRequestTags !== false,
    ...
    const createMemberRequestTag = async (config, sessionUser, {
        ...
        if (config.portalAllowRequestTags === false) {
            const err = new Error('Request tags are disabled.');
            err.status = 403;
            throw err;
        }
