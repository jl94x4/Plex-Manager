...
import { watcherTitleKey, isWatchingTitle, notifyEligibility } from './notifications/requestWatchers.js';
...
export const createRequestAppService = ({
    ...
    const submitMemberRequest = async (config, sessionUser, {
        ...
        if (plexToken) {
            ...
            if (session?.cookie) {
                // Cookie auth uses the member's Seerr permissions (no admin auto-approve).
                return fetchSeerrJson(config, '/api/v1/request', {
                    method: 'POST',
                    body,
        ...
        // API-key fallback still attributes the request, but Seerr auto-approves as the key owner.
