...
const preserveFileMetadata = async (filePath, stat) => {
    const warnings = [];
    try {
...
/** Append " [SMP]" before the extension so Replace produces a new sibling path for Plex. */
export const withSmpReplaceTag = (stem) => {
    const name = String(stem || '');
    if (!name) return '[SMP]';
    if (/\s*\[smp\]$/i.test(name)) return name.replace(/\s*\[smp\]$/i, ' [SMP]');
    return `${name} [SMP]`;
};
...
export const prepareMediaOutput = ({
    ...
    if (normalizedMode === 'copy') {
        ...
        const replaceStem = forceNewFilename ? withSmpReplaceTag(parsed.name) : parsed.name;
        finalPath = path.join(parsed.dir, `${replaceStem}${normalizedExtension}`);
    } else {
        finalPath = source;
    }
    return {
        ...
        sourcePath: source,
        finalPath,
        workPath: normalizedMode === 'dry-run' ? temporaryFor(source) : temporaryFor(finalPath),
    };
};
...
export const finalizeMediaOutput = async (prepared, {
    ...
    const preservationWarnings = sourceStat
        ? await preserveFileMetadata(prepared.workPath, sourceStat)
        : [];
    ...
    try {
        await renameFile(prepared.workPath, prepared.finalPath);
        if (sourceStat && prepared.finalPath !== prepared.sourcePath) {
