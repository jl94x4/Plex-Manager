...
export const prepareMediaOutput = ({
    ...
    const normalizedExtension = String(extension || parsed.ext || '.mkv').startsWith('.')
        ...
        : `.${extension}`;
    const finalPath = normalizedMode === 'copy'
        ? path.resolve(copyDestination || path.join(parsed.dir, `${parsed.name}.automated${normalizedExtension}`))
        : (normalizedMode === 'replace'
            ? path.join(parsed.dir, `${parsed.name}${normalizedExtension}`)
            : source);
    return {
        mode: normalizedMode,
        sourcePath: source,
