...
import { prepareMediaOutput, withSmpReplaceTag } from './output.js';
...
test('prepareMediaOutput supports dry-run copy and replace targets', () => {
    ...
    const tagged = prepareMediaOutput({
        sourcePath: '/media/Show - S01E01.mkv',
        mode: 'replace',
        extension: '.mkv',
        forceNewFilename: true,
    });
    assert.equal(tagged.finalPath, path.resolve('/media/Show - S01E01 [SMP].mkv'));
    assert.notEqual(tagged.finalPath, tagged.sourcePath);
    const alreadyTagged = prepareMediaOutput({
        sourcePath: '/media/Show - S01E01 [SMP].mkv',
        mode: 'replace',
        extension: '.mkv',
        forceNewFilename: true,
    });
    assert.equal(alreadyTagged.finalPath, path.resolve('/media/Show - S01E01 [SMP].mkv'));
    assert.equal(withSmpReplaceTag('a'), 'a [SMP]');
    assert.equal(withSmpReplaceTag('a [smp]'), 'a [SMP]');
});
...
test('config normalizes scan and watch settings', () => {
    ...
    assert.equal(defaults.replaceForceNewFilename, true);
