...
const collectIntelGpu = async ({ vaapiDevice = '/dev/dri/renderD128', devices = null } = {}) => {
    ...
    if (!top.available) {
        ...
        note = missingBin
            ? 'intel_gpu_top missing from this container image. Force-recreate/pull :nightly after the image rebuild that includes intel-gpu-tools. Encode via /dev/dri still works.'
            : (top.error
