...
const collectIntelGpuTop = async (deviceFilter = null) => {
    ...
    const result = await runCommand('intel_gpu_top', args, { timeoutMs: 2_800 });
...
const collectIntelGpu = async ({ vaapiDevice = '/dev/dri/renderD128', devices = null } = {}) => {
    ...
    if (biosLabel) prettyName = biosLabel;
    ...
    if (lspci.ok && deviceId) {
        ...
        if (line) {
            // Prefer PCI marketing name over weak BIOS labels like "Onboard IGD".
            prettyName = line.replace(/^[\da-f:.]+\s+/i, '').replace(/\s*\[[\da-f:]+\]\s*$/i, '').trim() || prettyName;
        }
    }
    if (/^onboard\s*igd$/i.test(String(prettyName || '').trim())) {
        prettyName = 'Intel Graphics (iGPU)';
    }
