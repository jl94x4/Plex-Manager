...
const collectIntelGpuTop = async (deviceFilter = null) => {
    ...
    let result = null;
    let lastError = 'intel_gpu_top unavailable';
    for (const binary of binaries) {
        // Need >1 sample period so busy% deltas are meaningful; kill after a few samples.
        result = await runCommand(binary, args, { timeoutMs: 2_800 });
    ...
    if (!sample) {
        return {
            ...
            error: result.stderr?.trim()
                || 'intel_gpu_top returned no usable JSON (needs perf access / intel-gpu-tools)',
            sample: null,
        };
    }
...
const collectIntelGpuTemperature = async (deviceRoot = null) => {
    const candidates = [];

    if (deviceRoot) {
        try {
            const hwmonRoot = path.posix.join(deviceRoot, 'hwmon');
...
const collectIntelGpu = async ({ vaapiDevice = '/dev/dri/renderD128', devices = null } = {}) => {
    ...
    let note = null;
    if (!top.available) {
        const missingBin = /ENOENT|not found|unavailable/i.test(String(top.error || ''));
        note = missingBin
            ? 'intel_gpu_top is not installed in this image yet — rebuild/update the portal image for live iGPU util. Encode via /dev/dri still works.'
            : (top.error
                ? `Live util unavailable: ${top.error}. Encode via /dev/dri still works.`
                : 'Install intel-gpu-tools (intel_gpu_top) and allow perf access for live engine utilization.');
    }
