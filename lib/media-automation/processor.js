...
import { discoverMediaFiles, listMediaFiles, resolveContainedPath } from './files.js';
...
export const createMediaProcessor = ({
    ...
    const enqueuePath = async (filePath, {
        ...
        if (!rule) {
            ...
            const reason = videoCodec && candidateRules.some((entry) => {
                ...
                : 'no-matching-rule';
            return { enqueued: false, reason, filePath: safePath, probe: { videoCodec }, hdr };
        }
    ...
    const processOne = async ({ lane, tags, priorityBias = 0, workerGroupId = null } = {}) => {
        ...
        try {
            ...
            if (settings.minSavingsPercent > 0
                ...
                if (guardSourceBytes > 0 && candidateBytes > 0) {
                    ...
                    if (savingsPercent < settings.minSavingsPercent) {
                        ...
                        await recordActivity({
                            type: 'job.skipped',
                            jobId: job.id,
                            message: `Kept original ${path.basename(sourcePath)}: encode saved ${Math.max(0, Math.round(savingsPercent))}% (minimum is ${settings.minSavingsPercent}%)`,
                            data: {
    ...
    const analyze = async ({
        ...
        const selectedLibraries = catalogLibraries.filter((library) => {
            if (library?.enabled === false || !library?.rootPath) return false;
    ...
    const estimate = async ({ filePath, pipelineId, sampleSeconds = 60 } = {}) => {
        const settings = await config();
        const catalogLibraries = typeof libraries?.list === 'function' ? await libraries.list() : [];
        ...
        const root = candidateRoots.find((entry) => {
            const relative = path.relative(path.resolve(entry), path.resolve(filePath));
            return relative === '' || (!relative.startsWith('..') && !path.isAbsolute(relative));
        });
        if (!root) throw errorWithCode('File is outside configured library roots', 'OUTSIDE_LIBRARY');
        const safePath = await resolveContainedPath(root, filePath, { allowSymlinks: false });
        const media = await probe(safePath, { ffprobePath: settings.ffprobePath });
        const durationSeconds = mediaDuration(media);
        ...
        try {
            await runner(settings.ffmpegPath, args, { timeoutMs: 15 * 60_000 });
