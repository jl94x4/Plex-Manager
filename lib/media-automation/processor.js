...
const FFMPEG_STEP_MODES = new Set(['transcode', 'remux', 'subtitle-strip', 'subtitle-extract']);
    ...
    const processOne = async ({ lane } = {}) => {
        ...
        try {
            ...
            if (skipFinalize) {
                ...
                const result = { plans, executions, output: { finalPath, skippedFinalize: true } };
                await queue.complete(job.id, workerId, result);
                await recordActivity({
                    type: 'job.completed',
                    jobId: job.id,
                    message: `Completed ${path.basename(sourcePath)}`,
                    data: { output: finalPath },
                });
                return { didWork: true, job, result };
            }
