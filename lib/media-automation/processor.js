...
export const createMediaProcessor = ({
    ...
    const analyze = async ({
        ...
        if (!selectedLibraries.length) {
            const requested = catalogLibraries.find((library) => String(library.id) === String(libraryId));
            if (requested && requested.enabled === false) {
                throw errorWithCode(`Library is disabled: ${libraryId}`, 'LIBRARY_DISABLED');
            }
            throw errorWithCode(
        ...
        const rows = await mapWithConcurrency(candidates, Math.min(8, Math.max(1, Number(concurrency) || 4)), async (entry) => {
            ...
            let media;
            try {
                media = await probe(entry.filePath, { ffprobePath: settings.ffprobePath, signal });
            } catch (error) {
                if (signal?.aborted || error?.code === 'ABORT_ERR') throw error;
                const detail = String(error?.stderr || error?.message || 'probe failed').trim().slice(0, 180);
                return buildAnalyzeRow({
                    filePath: entry.filePath,
                    library: entry.library,
                    probe: {
                        format: { filename: entry.filePath, size: entry.sizeBytes },
                        streams: [],
                    },
                    pipeline: forcedPipeline,
                    rule: null,
                    matched: false,
                    matchReason: `probe-failed: ${detail || 'Process exited with code 1'}`,
                });
            }
        ...
        const probeFailed = rows.filter((row) => String(row.matchReason || '').startsWith('probe-failed')).length;
        const totals = {
