...
const sidecarConfigPath = (configDir) => path.join(getCollexionsDataDir(configDir), 'config', 'config.json');

const readSidecarConfig = (configDir) => {
    try {
        return JSON.parse(fs.readFileSync(sidecarConfigPath(configDir), 'utf8'));
    } catch {
        return {};
    }
};

/** Copy portal Plex/TMDB credentials into the sidecar config when those fields are empty. */
export const seedSidecarConfigFromPortal = (portalConfig, { configDir, log = () => {} } = {}) => {
    if (!configDir) return false;
    ensureDataDirs(getCollexionsDataDir(configDir));
    const sidecar = readSidecarConfig(configDir);
    const plexUrl = String(portalConfig?.plexServerUrl || process.env.PLEX_SERVER_URL || '').trim().replace(/\/+$/, '');
    const plexToken = String(portalConfig?.plexToken || process.env.PLEX_TOKEN || '').trim();
    const tmdbKey = String(portalConfig?.tmdbApiKey || process.env.TMDB_API_KEY || '').trim();
    let changed = false;
    if (plexUrl && !String(sidecar.plex_url || '').trim()) {
        sidecar.plex_url = plexUrl;
        changed = true;
    }
    if (plexToken && !String(sidecar.plex_token || '').trim()) {
        sidecar.plex_token = plexToken;
        changed = true;
    }
    if (tmdbKey && !String(sidecar.tmdb_api_key || '').trim()) {
        sidecar.tmdb_api_key = tmdbKey;
        changed = true;
    }
    if (!changed) return false;
    fs.writeFileSync(sidecarConfigPath(configDir), JSON.stringify(sidecar, null, 4));
    log('[collexions] Seeded sidecar Plex credentials from portal settings');
    return true;
};
...
export const applyCollexionsBundledDefaults = (config, { configDir, log = () => {} } = {}) => {
    ...
    if (configDir) {
        ensureDataDirs(getCollexionsDataDir(configDir));
        seedSidecarConfigFromPortal(next, { configDir, log });
    }
...
const startWorker = async ({
    ...
    const env = {
        ...
        ...(plexUrl ? { COLLEXIONS_PLEX_URL: String(plexUrl).replace(/\/+$/, '') } : {}),
        ...(plexToken ? { COLLEXIONS_PLEX_TOKEN: String(plexToken) } : {}),
        ...(tmdbApiKey ? { COLLEXIONS_TMDB_API_KEY: String(tmdbApiKey) } : {}),
    };
...
export const syncCollexionsEmbeddedWorker = async (config, { configDir, log = () => {} } = {}) => {
    ...
    seedSidecarConfigFromPortal(config, { configDir, log });
    ...
    startingPromise = startWorker({
        ...
        plexUrl: String(config.plexServerUrl || process.env.PLEX_SERVER_URL || '').trim(),
        plexToken: String(config.plexToken || process.env.PLEX_TOKEN || '').trim(),
        tmdbApiKey: String(config.tmdbApiKey || process.env.TMDB_API_KEY || '').trim(),
        log,
    })
        .then(() => {
