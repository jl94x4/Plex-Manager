...
export const buildPathMountedArrTargetUrls = (
    ...
    const mount = String(mountSegment || '').replace(/^\/+|\/+$/g, '');
    const urls = [];
    const add = (href) => {
        if (href && !urls.includes(href)) urls.push(href);
    };
    const strippedSubPath = String(subPath || '').replace(/^\/+/, '');
    const subHasMount = mount && (strippedSubPath === mount || strippedSubPath.startsWith(`${mount}/`));
    const subWithoutMount = subHasMount
        ? strippedSubPath.slice(mount.length).replace(/^\/+/, '')
        : strippedSubPath;
    ...
    const pushVariants = (baseUrl) => {
        ...
        if (mount) {
            try {
                ...
                const baseHasMount = basePath.toLowerCase() === `/${mount.toLowerCase()}`
                    || basePath.toLowerCase().endsWith(`/${mount.toLowerCase()}`);
                const prefersRootFirst = /4k$/i.test(mount);
                const addOriginRootVariants = () => {
                    const originRoot = `${parsed.origin}/`;
                    add(buildCustomTabTargetUrl(originRoot, subPath, search, { stripMountPrefix: mount }));
                    if (!subHasMount && subWithoutMount) {
                        add(buildCustomTabTargetUrl(originRoot, subWithoutMount, search));
                    }
                };
                const rootStrip = () => {
                    add(buildCustomTabTargetUrl(baseUrl, subPath, search, { stripMountPrefix: mount }));
                    if (!subHasMount && subWithoutMount) {
                        add(buildCustomTabTargetUrl(baseUrl, subWithoutMount, search));
                    }
                };
                const mountPrefixed = () => {
                    if (baseHasMount) return;
                    const withMount = `${String(baseUrl).replace(/\/+$/, '')}/${mount}`;
                    if (subWithoutMount) {
                        add(buildCustomTabTargetUrl(withMount, subWithoutMount, search));
                    }
                    add(buildCustomTabTargetUrl(withMount, subPath, search, { stripMountPrefix: mount }));
                };
                if (prefersRootFirst) {
                    if (baseHasMount) addOriginRootVariants();
                    rootStrip();
                    mountPrefixed();
                } else {
                    mountPrefixed();
...
export const resolvePathMountedArrEmbedUpstream = (tabUrl, publicBaseUrl, arrInstances = []) => {
    try {
        ...
        return { upstreamUrl: internal, mountSegment };
    } catch {
        return null;
    }
};
