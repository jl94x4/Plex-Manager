...
export const createCustomTabEmbedProxyHandler = ({
    loadConfig,
    ...
    if (!tabId) {
        return res.status(400).send('Missing tab id');
    }

    try {
        const config = await loadConfig();
        const tabs = normalizeCustomNavTabs(config.customNavTabs);
        const tab = tabs.find((entry) => String(entry.id) === tabId);
        if (!tab || !tab.enabled || tab.openMode !== 'embed') {
            return res.status(404).send('Custom tab not found');
        }
        ...
        const targetUrl = buildCustomTabTargetUrl(tab.url, subPath, search);
        ...
        const proxyPath = withBasePath(`/api/custom-tab-embed/${encodeURIComponent(tabId)}`);
        ...
        if (upstream.status >= 300 && upstream.status < 400) {
            ...
            if (location) {
                res.setHeader('Location', rewriteLocationHeader(location, tab.url, proxyPublicPrefix));
            }
            return res.end();
        }
        ...
        const rewritten = rewriteProxiedBody(raw.toString('utf8'), contentType, tab.url, proxyPublicPrefix);
        return res.send(Buffer.from(rewritten, 'utf8'));
    } catch (error) {
        log(`[custom-tab-embed] ${tabId} ${req.method} failed: ${error.message}`);
        return res.status(502).send('Failed to load embedded tab through the portal proxy.');
    }
};
