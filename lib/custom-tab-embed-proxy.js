...
export const rewriteLocationHeader = (location, tabUrl, proxyPublicPrefix) => {
    try {
        ...
        if (!sameHost && resolved.origin !== base.origin) return location;
...
/** Map portal path mounts (e.g. /radarr) to internal LAN *arr URLs for proxy upstream fetches. */
export const resolvePathMountedArrEmbedUpstream = (tabUrl, publicBaseUrl, arrInstances = []) => {
    try {
        ...
        let arrType = PATH_MOUNTED_ARR_TYPES.has(mount) ? mount : null;
        if (!arrType) {
            const stripped = mount.replace(/4k$/, '');
            if (PATH_MOUNTED_ARR_TYPES.has(stripped)) arrType = stripped;
        }
        ...
        const instance = list.find((entry) => {
            if (!entry || entry.enabled === false) return false;
            const type = String(entry.type || '').toLowerCase().replace(/-4k$/, '4k');
            return type === arrType || type === `${arrType}4k`;
        });
        const internal = String(instance?.url || '').trim().replace(/\/+$/, '');
        if (!internal || !/^https?:\/\//i.test(internal)) return null;
        const internalHost = new URL(internal).hostname;
        if (!isPrivateIntegrationHost(internalHost)) return null;
        return { upstreamUrl: internal, mountSegment: pathParts[0] };
    } catch {
        return null;
    }
};
...
/** Detect direct iframe embeds at portal path mounts (e.g. /radarr/) from the Referer header. */
export const parsePathMountedArrFromReferer = (referer, publicBaseUrl = '') => {
    try {
        const ref = new URL(String(referer || '').trim());
        const segments = ref.pathname.replace(/\/+$/, '').split('/').filter(Boolean);
        if (segments.length !== 1) return null;
        const mount = segments[0].toLowerCase();
        const stripped = mount.replace(/4k$/, '');
        const arrType = PATH_MOUNTED_ARR_TYPES.has(mount)
            ? mount
            : (PATH_MOUNTED_ARR_TYPES.has(stripped) ? stripped : null);
        if (!arrType) return null;
        const publicBase = String(publicBaseUrl || '').trim();
        if (publicBase) {
            const pub = new URL(publicBase);
            if (ref.hostname.toLowerCase() !== pub.hostname.toLowerCase()) return null;
        }
        return { arrType, mountSegment: segments[0] };
    } catch {
        return null;
    }
};

export const buildPathMountedArrAssetTargetUrl = (reqPath, mountCtx, publicBaseUrl, arrInstances = [], search = '') => {
    const publicBase = String(publicBaseUrl || '').trim().replace(/\/+$/, '');
    const mountPath = `/${mountCtx.mountSegment}`;
    const tabUrl = `${publicBase}${mountPath}`;
    const assetSubPath = String(reqPath || '').replace(/^\//, '');
    const upstreamCtx = resolvePathMountedArrEmbedUpstream(tabUrl, publicBase, arrInstances);
    if (upstreamCtx?.upstreamUrl) {
        return buildCustomTabTargetUrl(upstreamCtx.upstreamUrl, assetSubPath, search, {
            stripMountPrefix: upstreamCtx.mountSegment,
        });
    }
    return buildCustomTabTargetUrl(tabUrl, assetSubPath, search);
};

export const createPathMountedArrAssetHandler = (deps) => async (req, res) => {
    const log = deps.log || (() => {});
    try {
        const config = await deps.loadConfig();
        const publicBase = deps.resolvePublicBaseUrlFromConfig?.(config) || '';
        const mountCtx = parsePathMountedArrFromReferer(
        ...
        const search = String(req.url || '').includes('?') ? req.url.slice(req.url.indexOf('?')) : '';
        const targetUrl = buildPathMountedArrAssetTargetUrl(
            req.path,
            mountCtx,
            publicBase,
        ...
        if (upstream.body) {
            return pipeUpstreamBody({ body: upstream.body }, res, {
                log,
                proxyRouteName: 'path-mounted-arr-asset',
                entityId: mountCtx.mountSegment,
            });
        }
        const raw = await readUpstreamBodyWithLimit(upstream, EMBED_PROXY_MAX_REWRITE_BYTES * 4);
        return res.send(raw);
    } catch (error) {
        log(`[path-mounted-arr-asset] ${req.method} ${req.path} failed: ${error.message}`);
        if (!res.headersSent) {
            res.status(502).send('Failed to load *arr asset');
        }
    }
};
...
export const createEmbedProxyHandler = ({
    ...
    try {
        ...
        let targetUrl;
        try {
            targetUrl = buildCustomTabTargetUrl(sourceUrl, subPath, search, { stripMountPrefix });
