...
const pickPathMountedArrInstance = (candidates, wants4k) => {
    if (!candidates.length) return null;
    const flagged = wants4k
        ? candidates.find((entry) => !!entry.is4k)
        : candidates.find((entry) => !entry.is4k);
...
export const isArrEmbedUpstreamFailure = (status, contentType = '') => {
...
export const resolvePathMountedArrEmbedUpstream = (tabUrl, publicBaseUrl, arrInstances = []) => {
    try {
        ...
        const candidates = list.filter((entry) => {
            ...
            return wants4k ? !!entry.is4k : !entry.is4k;
        });
        const instance = pickPathMountedArrInstance(candidates, wants4k);
...
export const createEmbedProxyHandler = ({
    ...
    try {
        ...
        for (const candidate of candidateUrls) {
            ...
            const failed = isArrEmbedUpstreamFailure(attempt.status, attemptType);
