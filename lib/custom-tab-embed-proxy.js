...
export const resolvePathMountedArrEmbedUpstream = (tabUrl, publicBaseUrl, arrInstances = []) => {
    try {
        ...
        const mountSegment = pathParts[0];
        ...
        if (!arrType && PATH_MOUNTED_ARR_TYPES.has(baseType)) arrType = baseType;
        ...
        const candidates = list.filter((entry) => {
            ...
            // Integrations use type "radarr" + is4k flag, not type "radarr4k".
            return wants4k ? !!entry.is4k : !entry.is4k;
        });
        const instance = wants4k
            ? (candidates.find((entry) => !!entry.is4k) || candidates[0])
            : (candidates.find((entry) => !entry.is4k) || candidates[0]);
        ...
        const internalPath = internalParsed.pathname.replace(/\/+$/, '');
        const mountLower = mountSegment.toLowerCase();
        const internalHasMount = internalPath.toLowerCase() === `/${mountLower}`
            || internalPath.toLowerCase().endsWith(`/${mountLower}`);
        // HD path mounts often use Url Base on LAN (append /radarr). 4K is usually a
        // dedicated port at / — public /radarr4k is Cloudflare-only, not on the app.
        let upstreamUrl = internal;
        if ((!internalPath || internalPath === '/') && !internalHasMount) {
            upstreamUrl = wants4k ? internal : `${internal}/${mountSegment}`;
        }
        return { upstreamUrl, mountSegment };
    } catch {
        return null;
    }
};
