...
export const notifyAdminPendingRequest = async ({
    ...
    // In-app bell for admins — deep-link into the Requests review modal.
    ...
    const notifBody = rendered.pushBody || rendered.gotifyBody
        || `${requester} has requested ${vars.media_type}: ${title}`;
    ...
    if (gotifyAllowed && typeof sendGotifyAlert === 'function') {
        try {
            gotifySent = !!(await sendGotifyAlert(
                ...
                rendered.gotifyBody || `${requester} has requested ${vars.media_type}: ${title}`,
            ));
        } catch (error) {
            if (typeof log === 'function') {
    ...
    return gotifySent || ntfySent || webhookSent || inAppCreated > 0;
};
