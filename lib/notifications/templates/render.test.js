...
test('scanner activity push copy keeps a short title and action in the body', () => {
    ...
    assert.equal(imported.rendered.pushTitle, 'Scanner Notification');
    assert.equal(imported.rendered.pushBody, `Imported: ${title}`);
    assert.equal(imported.rendered.ntfyTitle, 'Scanner Notification');
    assert.equal(imported.rendered.ntfyBody, `Imported: ${title}`);

    const upgrade = renderEventTemplates({}, 'scanner_upgrade', { title });
    assert.equal(upgrade.rendered.pushTitle, 'Scanner Notification');
    ...
    assert.equal(deleted.rendered.pushTitle, 'Scanner Notification');
    ...
    assert.equal(grabbed.rendered.pushTitle, 'Scanner Notification');
