/**
 * Fan-out when a portal request becomes available (slice 1a: email + in-app).
 */
        ...
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background-color: #f4f6f9; padding: 30px; color: #333333; line-height: 1.6;">
            <div style="max-width: 600px; margin: auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.05); border-top: 6px solid #e5a00d;">
                ...
                <div style="padding: 30px 40px;">
                    <h2 style="color: #e5a00d; font-size: 20px; margin-top: 0; font-weight: 600;">Your request is available</h2>
                    <p>Hello <strong>${safeName}</strong>,</p>
                    <p>Good news — the ${kind} you requested is ready to watch on the server.</p>
                    <div style="background-color: #fcf8f2; border-left: 4px solid #e5a00d; padding: 20px; margin: 25px 0; border-radius: 6px;">
                        ...
                        <p style="margin: 8px 0 0 0; color: #718096; font-size: 14px; text-transform: uppercase; letter-spacing: 0.06em;">Now available</p>
                    </div>
...
/**
 * @returns {{ notified: boolean, email: boolean, inApp: boolean, skipped?: string }}
 */
export const notifyRequestBecameAvailable = async ({
    ...
    if (!record?.id) return { notified: false, email: false, inApp: false, skipped: 'no-record' };
    if (!isRequestAvailableNotifyEnabled(config)) {
        return { notified: false, email: false, inApp: false, skipped: 'disabled' };
    }

    // Idempotent: already notified for this request.
    if (record?.meta?.notifiedAvailableAt) {
        return { notified: false, email: false, inApp: false, skipped: 'already-notified' };
    }

    const nextStatus = Number(record?.meta?.mediaStatus);
    if (nextStatus !== SEERR_MEDIA_AVAILABLE) {
        return { notified: false, email: false, inApp: false, skipped: 'not-available' };
    }

    const prev = Number(prevMediaStatus);
    if (prev === SEERR_MEDIA_AVAILABLE) {
        return { notified: false, email: false, inApp: false, skipped: 'was-already-available' };
    }
    ...
    if (shouldSendRequestAvailableInApp(config, user)) {
        ...
        if (targetUserId) {
            await createInAppNotification({
                ...
                body: 'Your request is ready to watch.',
                href,
                meta: {
                    requestId: record.id,
    ...
    if (emailTo && shouldSendRequestAvailableEmail(config, user) && typeof sendEmail === 'function') {
        ...
        if (!already) {
            try {
                ...
                const subject = `[${config?.serverName || 'Portal'}] ${title} is now available`;
    ...
    return {
        notified: emailSent || inAppCreated,
        email: emailSent,
        inApp: inAppCreated,
    };
};
