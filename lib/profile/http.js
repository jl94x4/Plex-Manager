...
import { normalizeMemberPrivacy } from '../privacy/memberPrivacy.js';
...
export const registerProfileRoutes = (app, deps) => {
    ...
    const buildForSubject = async ({
        ...
        const privacy = normalizeMemberPrivacy(portalUser || {});
        ...
        if (access.includeAccount) {
            try {
                const summary = await loadRequestSummary(req, config, portalUser || req.user);
                if (summary) {
                    requests = {
                        ...summary,
