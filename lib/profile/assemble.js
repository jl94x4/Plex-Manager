...
const tasteFromStats = (stats = {}) => {
    ...
    const genres = Object.entries(source)
        .filter(([key, value]) => (
            (key.startsWith('genreMovies_') || key.startsWith('genreShows_')) && Number(value) > 0
        ))
        .map(([key, value]) => ({
            id: key,
            label: key.replace(/^genre(?:Movies|Shows)_/, '').replace(/_/g, ' '),
            kind: key.startsWith('genreMovies_') ? 'movie' : 'show',
